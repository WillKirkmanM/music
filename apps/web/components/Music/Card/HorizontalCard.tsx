        ...
        <div className="ml-4">
          ...
          <div className="text-gray-400">
            {item_type == "song" && song_object && (
              <p>{artist_object?.name} • {album_object?.name} • {formatDuration(song_object.duration)}</p>
            )}
            {item_type == "album" && album_object && (
              <p>Album • {artist_object?.name} • {formatReleaseDate(album_object?.first_release_date ?? "")}</p>
            )}
            {item_type == "artist" && artist_object && (
              <p>Artist • {formatFollowers(artist_object?.followers ?? 0)} Followers</p>
            )}
          </div>
        </div>
      </div>
